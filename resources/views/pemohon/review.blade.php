                    ...
                    <div class="card">
                        ...
                        <div class="row">
                            <div class="col-6 ml-2 mt-2 mb-2">
                                <form action="{{ route('pemohon-storeakhir') }}" method="post"
                                    ...
                                    @foreach ($entry_data as $item)
                                        ...
                                        <input type="hidden" name="entry_data{{ $cocote }}[]"
                                            value="{{ $item }}" />
                                    @endforeach

                                    @foreach ($upload_data as $item)
                                        <input type="hidden" name="upload_data{{ $cocote }}[]"
                                            value="{{ $item }}" />
                                    @endforeach
                                    ...
                                    <form action="/pemohon/store" method="POST" enctype="multipart/form-data">
                                        ...
                                        <div class="modal-body p-4 bg-light">
                                            <div class="my-2">
                                                ...
                                                <input type="text" name="nik" class="form-control"
                                                    value="{{ $nik }}" placeholder="Masukan NIK" required>
                                            </div>
                                            <div class="my-2">
                                                <label for="no_kk">No KK</label>
                                                <input type="text" name="no_kk" class="form-control"
                                                    value="{{ $no_kk }}" placeholder="Masukan No KK" required>
                                            </div>
                                            <div class="my-2">
                                                <label for="nama_pemohon">Nama Pemohon</label>
                                                <input type="text" name="nama_pemohon" class="form-control"
                                                    value="{{ $nama_pemohon }}" placeholder="Masukan Nama Pemohon"
                                                    required>
                                            </div>
                                            <div class="my-2">
                                                <label for="alamat">Alamat</label>
                                                <textarea type="text" name="alamat"class="form-control" value="{{ $alamat }}"
                                                    placeholder="Masukan Alamat" required></textarea>
                                            </div>
                                            <div class="my-2">
                                                <label for="no_hp">No HP</label>
                                                <input type="text" name="no_hp" class="form-control"
                                                    value="{{ $no_hp }}" placeholder="Masukan No HP" required>
                                            </div>
                                            <div class="my-2">
                                                <label for="email">Email</label>
                                        ...
                                        @foreach (\App\Models\Rincian_layanan::with('layanan', 'persyaratan')->where('id_layanan', $id_layanan)->get() as $item => $value)
                                            ...
                                            @if ($value->persyaratan->entry_data != null && $value->persyaratan->upload_data != null)
                                                <div class="row">
                                                    <label class="mt-2"
                                                        for="">{{ $value->persyaratan->nama_persyaratan }}</label>
                                                    <div class="col-6">
                                                        <input type="text" name="entry_data{{ $cocote }}[]"
                                                            value="" class="form-control" />
                                                    </div>
                                                    <div class="col-6">
                                                        <input type="file" name="upload_data{{ $cocote }}[]"
                                                            value="" class="form-control" />
                                                    </div>
                                                </div>
                                            @elseif($value->persyaratan->upload_data == 1 && $value->persyaratan->entry_data == null)
                                                <label class="mt-2"
                                                    for="">{{ $item->persyaratan->nama_persyaratan }}</label>
                                                <input type="file" name="upload_data{{ $cocote }}[]"
                                                    class="form-control" />
